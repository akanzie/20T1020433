...
namespace _20T1020433.DataLayers.SQLServer
{
    ...
    public class OrderDAL : _BaseDAL, IOrderDAL
    {
        ...
        public IList<Order> List(int page = 1, int pageSize = 0, int status = 0, int shipperID = 0, string searchValue = "")
        {
            List<Order> data = new List<Order>();
            if (searchValue != "")
                searchValue = "%" + searchValue + "%";
                ...
                cmd.CommandText = @"SELECT  *
                                    FROM    (
                                            ...
                                            FROM    Orders as o
                                                    ...
                                                    LEFT JOIN Shippers AS s ON o.ShipperID = s.ShipperID
                                            WHERE   (@Status = 0 OR o.Status = @Status)
                                                 AND (@ShipperID = 0 OR o.ShipperID = @ShipperID)
                                                AND (@SearchValue = N'' OR c.CustomerName LIKE @SearchValue OR s.ShipperName LIKE @SearchValue)
                                            ) AS t
                                    WHERE (@PageSize = 0) OR (t.RowNumber BETWEEN(@Page -1)*@PageSize + 1 AND @Page*@PageSize)
                                    ORDER BY t.RowNumber";
        ...
        public int Count(int status = -99, int shipperID = 0, string searchValue = "")
        {
            int count = 0;
            if (searchValue != "")
                searchValue = "%" + searchValue + "%";
